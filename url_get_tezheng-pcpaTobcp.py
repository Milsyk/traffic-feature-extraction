...
def capture_and_get_features_frompcap(url_list, result, f):
    global HTTP_PORT
    global HTTPS_PORT
    TCP = 'TCP'
    # global group_n
    ...
    pcap_dir = 'C:/Users/Administrator/Desktop/analysis-2'
    # 遍历目录中的所有文件
    for dirpath, dirname, filenames in os.walk(pcap_dir):
        for filename in filenames:
            if filename.endswith('.pcap'):
                group_n = filename.split("_")[0]
                ...
                # # time.sleep(1)
                # # 保存数据包
                # pcap_name = str(group_n) + '_' + str(url_n)
                # print(pcap_name)
                # wrpcap("C:/Users/Administrator/Desktop/analysis/" + pcap_name + ".pcap", packets)
                # url = url_list[int(group_n) - 1]
                # deal_packets(packets, url, result)
                ...
                for five_tuple, packets_f in packets_by_five_tuple.items():
                    ...
                    # time.sleep(1)
                    url = url_list[int(group_n) - 1]
                    deal_packets(packets_f, url, result)
                    output(result, group_n, f)
                    wrpcap("C:/Users/Administrator/Desktop/analysis/" + str(group_n) + '_' + str(url_n) + ".pcap",
                           packets_f)
                    # g_n += 1
                    result = []
...
if __name__ == "__main__":
    ...
    capture_and_get_features_frompcap(url_list, result, f)
        # t_getUrl.start()
