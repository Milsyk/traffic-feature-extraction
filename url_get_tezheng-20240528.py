...
def packet_sniff():
    ...
    # wrpcap("C:/Users/Administrator/Desktop/" + , packets)
    # return packets
...
def get_url_list(url_request,result):
    # 要请求的URL
    # 发送请求并获取响应
    try:
        proxy_http_address = 'http://127.0.0.1:9999'
        ...
        check_packets(packets)
            ...
            # else:
                # time.sleep(5)
            # time.sleep(5)
        # global error_url_num
        if error_flag:
            # wrpcap("C:/Users/Administrator/Desktop/analysis/" + str(group_n) + '_' + '1' + "测试.pcap", packets)
            correct_pkts = correct_packets(packets)
            packets = correct_pkts
            # wrpcap("C:/Users/Administrator/Desktop/analysis/" + str(group_n) + '_' + '1' + "测试纠正!!!!!.pcap", correct_pkts)
            # with open(r'C:\Users\Administrator\Desktop\error_packet_urlNum.bcp', 'a') as f:
            #     # 向文件追加内容
            #     f.write('\n' + str(group_n) + '_' + '1')

        #     with open(r'C:\Users\Administrator\Desktop\error_packet_urlNum.bcp', 'w', newline='') as f:
        #         print(str(group_n) + '_' + '1', file=f)
        #         print('\n', end="", file=f)
        error_flag = False
        # time.sleep(1)
        deal_packets(packets, url_request, result)
        wrpcap("C:/Users/Administrator/Desktop/analysis/" + str(group_n) + '_' + '1' + ".pcap", packets)
        # 解析HTML文本
        ...
        for link in soup.find_all('link'):
            ...
            # 如果href不为空
            if href:
                # 如果字符串的第一个字符不是'/'，则在前面添加'/'
                if not href.startswith('/') and not href.startswith('http'):
                    href = '/' + href
                    # 如果字符串的前两个字符都是'/'，则去掉第一个'/'
                if href.startswith('//'):
                    href = href[1:]
                    # 将链接地址添加到列表中
                if len(href.split('.css')) > 1:
                    if len(href.split("https://")) <= 1 and len(href.split("http://")) <= 1:
                        href_exe = url_request + href
                        # print(href_exe)
                        links.append(href_exe)
                    else:
                        links.append(href)
        ...
        for link in soup.find_all('script'):
            ...
            if src:
                ...
                if src.startswith('//'):
                    src =src[1:]
                # 将链接地址添加到列表中
                if len(src.split('.js')) > 1:
                    if len(src.split("https://")) <= 1 and len(src.split("http://")) <= 1:
                        src_exe = url_request + src
                        # print(src_exe)
                        links.append(src_exe)
                    else:
                        links.append(src)
...
if __name__ == "__main__":
    ...
    with open(r'C:\Users\Administrator\Desktop\url_feature.bcp', 'w', newline='') as f:
        ...
        for URL in url_list:
            # 要请求的URL
            result = []
            links = get_url_list(URL, result)
            links = set(links)
            # 多线程开启

            t_analysis_feature = threading.Thread(target=capture_and_get_features, args=(links, result))
            t_analysis_feature.start()

            t_analysis_feature.join()
