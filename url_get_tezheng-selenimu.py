...
def get_url_list(url_request, result, browser, group_n, f):
    # 要请求的URL
    # 发送请求并获取响应
    try:
        proxy_http_address = 'http://127.0.0.1:9999'
...
if __name__ == "__main__":
    ...
    for content in file_contents:
        ...
        # print(split_cont)
        if len(split_cont) > 1:
            content = split_cont[1].split('http://')
            if len(content) > 1:
                url_list.append(deepcopy(split_cont[1]))
            else:
    ...
    # 设置360浏览器的WebDriver路径
    # webdriver_path = 'C:\\path\\to\\360\\webdriver\\chromedriver.exe'
    # 指定ChromeDriver路径
    chrome_service = webdriver.ChromeService(executable_path='D:\zqyproject\流量特征分析\chromedriver-win64-122\chromedriver-win64\chromedriver.exe')
    # 设置360浏览器的可执行文件路径
    browser_path = 'C:\\Users\\Administrator\\AppData\\Roaming\\360se6\\Application\\360se.exe'
    # 配置360浏览器的可执行路径
    browser_options.binary_location = browser_path
    # 注意options的参数用之前定义的chrome_options
    # browser = webdriver.Chrome(options=browser_options)
    browser = webdriver.Chrome(options=browser_options, service=chrome_service)
    ...
    for URL in url_list:
        # 要请求的URL
        result = []
        t_getUrl = threading.Thread(target=get_url_list, args=(URL, result, browser, group_n, f))
        t_getUrl.start()
